...
void findVAlpha() {
    ...
    float temp[NUM_STATES][NUM_STATES];
    for (i = 0; i < NUM_STATES; ++i) {
        for (j = 0; j < NUM_ACTIONS; ++j) {
            temp[i][j] = I[i][j] - (ALPHA * Pf[i][j]);
        }
    }

    for (i = 0; i < NUM_STATES; ++i) {
        for (j = 0; j < NUM_ACTIONS+1; ++j) {
            if (j == NUM_ACTIONS) a[i+1][j+1] = r[i];
            else a[i+1][j+1] = temp[i][j];
        }
    }
    ...
    for(k=1; k <= NUM_STATES - 1; k++) {
        if(a[k][k]==0.0) {
            ...
            pivot = a[k][k];
            for(j=k; j <= NUM_STATES + 1; j++) a[k][j]= a[k][j]/pivot;
            for(i=k+1; i <= NUM_STATES; i++) {
                factor = a[i][k];
                for(j = k; j <= NUM_STATES + 1; j++) {
                    a[i][j] = a[i][j] - factor * a[k][j];
                }
            }
        }

        if(a[NUM_STATES][NUM_STATES] == 0) printf("error");
        else {
            x[NUM_STATES] = a[NUM_STATES][NUM_STATES + 1] / a[NUM_STATES][NUM_STATES];
            for(i= NUM_STATES - 1; i >= 1; i--) {
                sum = 0.0;
                for(j=i+1; j <= NUM_STATES; j++)
                    sum = sum + a[i][j] * x[j];
                x[i]= (a[i][NUM_STATES + 1] - sum ) / a[i][i];
            }
        }
    }
    for(i=1; i <= NUM_STATES; i++) V[i-1] = x[i];
}
...
int findA() {
    ...
    for (i = 0; i < NUM_STATES; i++) {
        for (a = 0; a < NUM_ACTIONS; a++) {
            val = R[i][a];
            for (j = 0; j < NUM_STATES; j++) {
                val += ALPHA * T[i][a][j] * V[j];
            }
        if (val > V[i]) optimale[i][a] = 0;
        else optimale[i][a] = 1;
        }
    }
